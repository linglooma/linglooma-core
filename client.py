...
def run():
    ...
    with open("/home/xuananle/Documents/Linglooma/Linglooma-core/resources/audio/part2-1.mp3", "rb") as f:
        audio_data = f.read()
    request = SpeakingAssessmentRequest(audio=audio_data)
    response = stub.AssessSpeaking(request)
    print(json.dumps(MessageToDict(response, preserving_proto_field_name=True), indent=4, ensure_ascii=False))
